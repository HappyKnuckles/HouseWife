...
`id`, `household_id`, `barcode text` (EAN-8/13/UPC, nullable for unbarcoded items), `name`, `brand`, `category`, `unit text check (unit in ('piece','g','kg','ml','l','pack'))`, `net_quantity numeric`, `image_path`, `default_location_id`, `notes`, `kind text check (kind in ('consumable','equipment'))`, `restock_min_quantity numeric`, `source text check (source in ('manual','scan','external'))`, `external_provider text`, `external_id text`, `external_payload jsonb`, timestamps.
`unique (household_id, barcode) where barcode is not null`.

**`kind` — Vorräte vs. Ausstattung** (migration 0032). A `'consumable'` is eaten or used up: quantity falls and `restock_min_quantity` can put it on the Einkaufsliste. An `'equipment'` is owned — a Akkuschrauber does not run out, it is either where it belongs or somebody left it in the Bad. So it carries no threshold (`check (kind = 'consumable' or restock_min_quantity is null)`, enforced rather than merely avoided in the UI, because `v_inventory_totals.is_low` reads both columns and two sources of truth that can disagree is how a Bohrmaschine ends up on the shopping list) and instead gets a **fester Platz**.

That fester Platz is `default_location_id`, not a new column. The column already meant "where this gets put away when nobody says otherwise", which is the same fact read from the other side: for a Vorrat it answers *wohin damit*, for a tool it also answers *wo müsste es sein*. A second location column would mean two answers that can contradict each other. Like `restock_min_quantity` it lives on the product rather than the lot, because `inventory_adjust()` deletes an emptied lot and a tool that is currently lent out must not lose the memory of where it belongs.

`inventory_scan_in(… , p_kind)` applies the kind only when it *creates* a product — a barcode or name match keeps the entry's existing kind, since the match is about identity, not category. `p_kind` NULL is meaningfully different from `'consumable'`: a named kind also narrows the name match to that kind (so the Vorrat "Batterien" and the Ausstattung "Batterien" stay two things), while NULL lets the match reach across both. The Einkauf checkout books hand-written rows in with NULL for exactly that reason — buying a replacement Akkuschrauber should land on the one you already own, fester Platz included, rather than fork a second entry.
...
It also answers both kinds' headline question in one row: `is_low` (a `'consumable'` whose total has reached `restock_min_quantity` — always false for equipment, which is what keeps tools off the Einkaufsliste) and `is_misplaced` (an `'equipment'` with a fester Platz holding stock somewhere else). `is_misplaced` is false when `default_location_id` is NULL: "noch kein Platz vereinbart" is not the same claim as "am falschen Platz", and flagging every newly added tool would make the signal worthless.
